...
function ContactComponent() {
	...
	const phoneHref = details.generalPhone
		...
		: undefined;
	const mapLatitude = Number.isFinite(details.mapLatitude)
		? details.mapLatitude
		: defaultDetails.mapLatitude;
	const mapLongitude = Number.isFinite(details.mapLongitude)
		? details.mapLongitude
		: defaultDetails.mapLongitude;
	const mapZoom = Number.isFinite(details.mapZoom)
		? details.mapZoom
		: defaultDetails.mapZoom;
	...
	return (
		...
		>
			<div className="max-w-7xl mx-auto">
				...
				<div className="mb-20">
					<h2
						className={`text-5xl md:text-6xl text-primary-500 mb-6 ${
							locale === "persian"
					...
					<div
						className={`h-1.5 w-32 bg-secondary-500 rounded-full ${locale === "persian" ? "mr-0" : "ml-0"}`}
					/>
				</div>
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24">
					...
					<div className="flex flex-col gap-10">
						...
						<div className="grid gap-6">
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
								...
								<div className="group p-6 rounded-2xl bg-white border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
									...
									<h3
										className={`text-lg font-bold text-primary-500 mb-2 ${locale === "persian" ? "font-bodyFa" : "font-header"}`}
									>
										{locale === "persian" ? "رسانه" : "Media Inquiries"}
									</h3>
									<a
										href={`mailto:${details.mediaEmail}`}
										className="text-project-gray-400 hover:text-secondary-500 transition-colors font-body text-sm"
									>
										{details.mediaEmail}
									</a>
								</div>
								...
								<div className="group p-6 rounded-2xl bg-white border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
									...
									<div className="flex flex-col gap-1">
										<a
											href={phoneHref}
											dir="ltr"
											className="text-project-gray-400 hover:text-secondary-500 transition-colors font-body text-sm text-left"
										>
											{details.generalPhone}
										</a>
										<a
											href={`mailto:${details.generalEmail}`}
											className="text-project-gray-400 hover:text-secondary-500 transition-colors font-body text-sm"
										>
						...
						<div className="mt-8 pt-10 border-t border-gray-100">
							<div className="mb-8">
								<h3
									className={` text-primary-500 mb-4 ${locale === "persian" ? "font-bodyFa" : "font-body"}`}
								>
							...
							<div className="flex gap-4">
								{[
									{
										Icon: Twitter,
										link: `https://twitter.com/${text.general.social_media_usernames.twitter}`,
									},
									{
										Icon: Instagram,
										link: `https://www.instagram.com/${text.general.social_media_usernames.instagram}`,
									},
									{
										Icon: Youtube,
										link: `https://youtube.com/${text.general.social_media_usernames.youtube}`,
									},
								].map(({ Icon, link }) => (
									<Link
										key={link}
										href={link}
										target="_blank"
										className="w-12 h-12 rounded-full border border-gray-200 flex items-center justify-center text-project-gray-400 hover:bg-secondary-500 hover:text-white hover:border-secondary-500 transition-all duration-300 group"
									>
						...
						<div className="absolute bottom-8 left-8 right-8 pointer-events-none flex justify-end">
							...
							>
								<div className="text-left rtl:text-right">
									<p
										className={`text-xs text-project-gray-400 font-semibold uppercase tracking-wider mb-1 ${locale === "persian" ? "font-bodyFa" : "font-body"}`}
									>
										{text.contact.map.button}
									</p>
									<p
										className={`text-primary-500 font-semibold ${locale === "persian" ? "font-bodyFa" : "font-body"}`}
									>
										Open in Google Maps
									</p>
								</div>
