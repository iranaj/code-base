...
export default function AdminSidebarLayout({ children }: { children: React.ReactNode }) {
  ...
  const navigation = [
    ...
    { name: 'Advocacy', href: `/${locale}/admin/advocacy`, icon: MegaphoneIcon },
    { name: 'Events', href: `/${locale}/admin/events`, icon: CalendarDateRangeIcon },
    { name: 'Hero Slides', href: `/${locale}/admin/hero-slides`, icon: RectangleStackIcon },
    { name: 'Contact Details', href: `/${locale}/admin/contact-details`, icon: PhoneIcon },
          ...
          {navigation.map((item) => {
            ...
            return (
              <Link
                ...
                `}
              >
                <item.icon className={`w-6 h-6 ${isActive ? 'text-primary-900' : 'text-project-gray-400'}`} />
                {item.name}
              </Link>
            );
          })}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-20 bg-white border-b border-project-gray-200 flex items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-4 ml-auto">
             <span className="text-sm font-medium text-project-gray-500">Admin Portal</span>
             <UserButton 
                appearance={{
                  elements: {
                    userButtonAvatarBox: 'w-10 h-10 border border-project-gray-200'
                  }
                }}
             />
          </div>
