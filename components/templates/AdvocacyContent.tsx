						...
						{items.map((item: any, index: number) => {
							...
							return (
								<div key={item._id} className="relative group">
									<div
										className={`absolute top-2 -left-[9px] w-4 h-4 rounded-full border-2 border-white bg-project-gray-300 group-hover:bg-secondary-500 group-hover:scale-125 transition-all duration-300 z-10 ${
									...
									<Link
										...
										className={`block py-2 pl-8 pr-4 text-sm !text-project-gray-500 font-semibold group-hover:!text-primary-500 transition-colors duration-300 ${
											isPersian
												? "font-bodyFa text-right pr-8 pl-4"
												: "text-left"
										}`}
									>
			...
			>
				...
				<div className="flex flex-col gap-16">
					{items.map((item: any, index: number) => {
						...
						const dateStr = new Date(item.publishedAt).toLocaleDateString(
						...
						return (
							...
							>
								{/* Date Label */}
								<div
									className={`absolute -top-6 left-8 z-20 ${isPersian ? "right-8 left-auto" : ""}`}
								>
