	...
	try {
		...
		if (existingCount > 0) {
			return NextResponse.json(
				{ error: "Hero slides already exist" },
				{ status: 409 },
			);
		}
		...
		const payload = Array.from({ length: max })
			.map((_, index) => {
			...
			.filter(
				(item) =>
					item.text.en.trim().length > 0 || item.text.fa.trim().length > 0,
			);

		if (payload.length === 0) {
			return NextResponse.json(
				{ error: "No default slides found to import" },
				{ status: 400 },
			);
		}
