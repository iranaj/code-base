...
export default function UsersPage() {
	...
	const toggleAdminRole = async (userId: string, currentRole?: string) => {
		if (userId === currentUser?.id) {
			if (
				!confirm(
					"Warning: You are about to remove your own admin privileges. You will lose access to this dashboard immediately. Are you sure?",
				)
			) {
				return;
			}
		...
		try {
			...
			if (response.ok) {
				toast.success(
					`User role updated to ${newRole ? "Admin" : "Regular User"}`,
				);
			} else {
								...
								{users.map((user) => {
									...
									return (
										<tr key={user.id} className="hover:bg-gray-50">
											...
											<td className="px-6 py-4 whitespace-nowrap">
												<div className="text-sm text-project-gray-500">
											...
											<td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
												<button
													type="button"
													onClick={() =>
														toggleAdminRole(user.id, user.publicMetadata?.role)
													}
													className={`
