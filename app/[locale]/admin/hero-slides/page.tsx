  ...
  const fetchItems = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setItems(data);
        if (Array.isArray(data) && data.length === 0 && !attemptedSeed) {
          seedSlides();
        }
      } else {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold font-header text-primary-900">Manage Hero Slides</h1>
        <Link
        ...
        {loading ? (
          ...
          <table className="min-w-full divide-y divide-project-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-project-gray-500 uppercase tracking-wider">Slide Text (English)</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-project-gray-500 uppercase tracking-wider">Order</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-project-gray-500 uppercase tracking-wider">Status</th>
            ...
            <tbody className="bg-white divide-y divide-project-gray-200">
              {items.map((item) => (
                <tr key={item._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-project-gray-500">{item.order ?? 0}</td>
